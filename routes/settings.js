...
router.get('/', requireAdminAuth, (req, res) => {
  const row = db.prepare('SELECT notify_email, site_title FROM app_settings WHERE id = 1').get();
  res.json(row);
});
...
router.put('/', requireAdminAuth, (req, res) => {
  ...
  db.prepare('UPDATE app_settings SET notify_email = ?, site_title = ? WHERE id = 1').run(
